...
def _render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- 完成结构化判断：{stats.get('analysis_completed_count', stats.get('analysis_count', 0))}",
        f"- 抽取结构失败、保留待重试：{stats.get('analysis_failed_count', 0)}",
        f"- 用户 safety limit 延后抽取：{stats.get('analysis_safety_deferred_count', 0)}",
        f"- 软时间预算延后抽取：{stats.get('analysis_budget_deferred_count', 0)}",
        f"- 待抽取 backlog 合计：{stats.get('analysis_deferred_count', 0)}",
        f"- 恢复待深挖路线：{stats.get('pending_deep_backlog_loaded', 0)}",
        f"- 计划深挖候选：{stats.get('planned_deep_candidate_count', 0)}",
        f"- 完成深挖候选：{stats.get('deep_candidate_count', 0)}",
