...
上游的 `query_group`、`origin_classification_reason` 和 `document_format` 只说明材料如何被发现、为什么获得完整文档分析资格，不证明其中一定有新范式。重点研究者车道命中不能替代机制证据；System Card、Model Card、Whitepaper 也可能只有评测或治理信息。反过来，品牌化标题没有已知技术关键词时，也不能因为分类器初步领域为空就丢掉正文里真实存在的 intervention。
