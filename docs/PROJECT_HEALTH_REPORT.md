...
> 2026-07-19 第三次更新。两轮 GitHub Actions 均已成功运行并完成邮件投递；本轮新增 Tavily 与 Reddit 社区覆盖，只进行了本地编译、模拟与单元测试，没有再次请求真实外部 API 或发送邮件。
...
- Python 静态编译通过。
- 29 项本地单元测试通过，新增覆盖 Technical Report 多机制拆解与专项查询降级、强组织优先准入、未知发布者二次验证、作者自发帖排除、真实相关 GitHub 仓库承接、观察池跨周刷新、官方研究页召回、Tavily 三平台识别与禁止计分、Reddit OAuth/互动指标、社区正文清除、英文长段拒绝和禁止字段拼装降级报告。
