...
- 2026-07-31 的例行任务不是死在某个证据接口：旧 artifact 因 schema 元数据不一致被直接丢弃，触发 60 天冷启动；发现层返回 17,516 条原点后，全量 `asyncio.gather` 试图在一个 job 内完成，最终被 Actions 的 90 分钟硬超时取消。硬取消绕过了报告、状态发布与主流程内邮件，所以用户没有收到任何提醒。当前实现改为兼容状态迁移、发现结果先入 backlog、按软时间预算分批检查点、优先处理高势能原点，并增加独立于主流程的失败邮件步骤。时间预算只控制执行进度，不改变 Rubric 或候选去留。
- 日志中的 Qwen JSON 截断、Rubric 覆盖不足和单次请求超时是吞吐放大器，不是最终退出原因。单条结构失败现在继续留在 pending，并保留跨周失败次数；同优先级下先让未尝试报告获得机会，不能让一个反复失败的报告长期占住队首，也不能把结构失败写成技术淘汰。已分析材料若正文发生变化，状态会明确重置为 pending，避免恰逢预算边界时静默丢失新版本。
